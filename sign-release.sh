...
sed -i 's!Packages.new!Packages!g' "$1"
gpg --no-tty --batch --passphrase-fd=0 --default-key "$KEYID" --detach-sign -o Release.gpg.tmp "$1"
cp Packages.new Packages
cp Packages.new.gz Packages.gz
cp Packages.new.bz2 Packages.bz2
mv Release.dinstall-new Release
mv Release.gpg.tmp Release.gpg
#rm -f Packages.new* Release.new*
